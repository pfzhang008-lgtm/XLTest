      ...
      <view class="content-area">
        ...
        <view class="action-group">
          ...
          <button class="btn-primary" @click="handleNext" hover-class="btn-hover">
            <text class="btn-text">{{ isReviewMode ? '返回评估报告' : '进入下一项测试' }}</text>
            <text class="material-icons btn-icon">{{ isReviewMode ? 'arrow_back' : 'arrow_forward' }}</text>
          </button>
...
export default {
  ...
  methods: {
    ...
    handleNext() {
      // Always navigate back to Briefing page which acts as the central hub
      // The Briefing page's onShow will detect the step update and unlock the next test
      uni.navigateBack();
    },
    parsePayload(payload) {
      ...
      if (payload.metrics) {
          ...
          this.accuracy = payload.metrics.accuracy || 0;
          
          // SART specific
          if (this.testType === 'SART') {
             if (payload.metrics.falseAlarms !== undefined) {
                 this.errors = payload.metrics.falseAlarms + payload.metrics.omissions;
             }
          }
      } else {
    ...
    onLoad(options) {
      ...
      if (this.step && this.retestUrl) {
          this.retestUrl += `&step=${this.step}`;
      }
      
      // Cache result if not in review mode (or update it?)
      // Actually we don't need to re-cache if we just loaded it from cache.
      // But if it was passed via options.data (standalone), we might want to cache.
      if (!this.step && this.moduleId && this.testType) {
