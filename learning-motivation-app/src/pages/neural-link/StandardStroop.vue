...
export default {
  ...
  onLoad(options) {
    ...
    if (options && options.step) {
      this.step = parseInt(options.step) || 4;
    }
    ...
    this.config = getNormsByAge(userAge, 'standard_stroop');
    ...
    finishGame() {
      ...
      try {
        ...
        if (this.moduleId) {
          ...
          if (isNaN(currentStepNum)) currentStepNum = 4; // Default to step 4 for Standard Stroop in Mod 03
